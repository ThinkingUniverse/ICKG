...
> **使用场景 / 解决的问题**：要厘清一条免疫机制（如"IL-6 怎样一步步导致类风湿关节炎加重？"）往往要人工读几十篇综述。本场景利用 ICKG 的 17 万+ 有向因果边，**自动搜出"因子→细胞→表型→疾病"的多跳因果链并按证据强度排序**，再由大模型改写成可读叙事——把数周的人工综述压缩成秒级的机制假设草图，供专家审阅。
